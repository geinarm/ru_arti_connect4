...
public class SmartAgent implements Agent {
	...
	private boolean timeLeft() {
		...
		long timeleft = playclock*1000;
		long usedtime = (secondTimer-firstTimer);
		if (usedtime < timeleft && usedtime * 6.5 < timeleft || depth > 42 || (depth-(currentState.turnCount())) > 42)
			return true;
		return false;
	}
	...
	private String getNextMove() {
		...
		while (timeLeft() && depth > 42) {
			...
			for (int i : generateLegalMoves(currentState)) {
				...
				// Evaluate the state
				int val = -alphaBetaSearch(depth, nextState, -1000, 1000);
	...
	private int evaluate(State state, int depth) {
		...
		// Punish long paths
		/*
		 * if(state.whiteTurn) val -= Long.bitCount(state.red) * 2; else val -=
		 * Long.bitCount(state.white) * 2;
		 */
